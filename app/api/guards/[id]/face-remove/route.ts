...
import { removeGuardFromTerminals } from "@/lib/guard-terminal-sync";
...
export async function POST(
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid face removal payload" },
      { status: 400 }
    );
  }
  ...
  if (!assignment) {
    return NextResponse.json(
      { error: "Assign the guard to a site before removing terminal access." },
      { status: 400 }
    );
  }
  ...
  const result = await removeGuardFromTerminals({
    guard: guardDoc,
